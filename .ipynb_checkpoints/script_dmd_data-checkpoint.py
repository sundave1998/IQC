...
#     lambda_2 = 0.01
    ...
    rho_selection = np.linspace(0.9,1,resolution,endpoint=False)
...
#                 prob.solve(solver=cp.MOSEK)
    ...
    plt.plot(eta_selection, 1-(0.1* IQC_rho_plot.T)/(resolution))
#     plt.imshow(np.flip(np.transpose(viable), axis=0))
#     plt.show()

IQC_rho_plot = 1-(0.1*IQC_rho_plot)/(resolution)
np.save("IQC_rho_plot_f1612", IQC_rho_plot)